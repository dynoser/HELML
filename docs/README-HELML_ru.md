...
Кодировщик HELML позволяет закодировать данные в специальном URL-режиме, когда разделители строк заменяются на "`~`",
управляющие двоеточия на "`.`", а управляющие пробелы на нижнее подчёркивание "_".
На выходе получается строка, которая может быть использована как GET-параметр для URL,
...
Это простой способ передать в одной строке произвольный массив с сохранением его структуры и типов данных.
