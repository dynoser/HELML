...
Эти символы могут быть заменены другими. Например, для кодировки URL удобно заменить их на "`.`" и "`_`",
поскольку преобразование "urlencode" (RFC 3986) их не изменяет. При такой замене постфикс будет: `~#._~`.

Итак, конец HELML-разметки может обозначать либо `~#: ~`, либо `~#._~` для однострочного URL-режима.
