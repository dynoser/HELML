...
HELML позволяет создавать вложенные массивы добавляя слева знаки `':'`, например:
```sh
Hosts:
 :--: github.com
 :--: www.github.com
...
стандартный упаковщик JSON будет "спотыкаться". HELML поможет, он работает с любыми данными.
...
Формате HELML легко создавать и редактировать вручную.
...
- Строки, начинающиеся со знака "`#`", игнорируются - это комментарии.
...
### Вложенностью массивов управляет знак "`:`" слева.

Количество двоеточий в начале строки означает целевой уровень вложенности.

Рассмотрим простой пример. Закодируем это в HELML:
```json
{
	"Host": "github.com",
	"Names": {
...
Любая строка HELML (кроме пустых и закомментированных) имеет ключ и, возможно, что-то ещё.
...
и в таком случае речь идёт о корневом уровне массива. Это нулевой (корневой) уровень вложенности.
...
## -- *Заметим, что выше была изложена основная концепция HELML, и ниже речь будет идти о нюансах*
...
Вложенные массивы создаются указанием "ключа" и пустого значения, и здесь возможны два варианта:
  ...
  2) ключ и "разделяющее двоеточие", после которого нет значения.
...
List:
 ...
 :2: C
```
Если это двоеточие убрать, то массив будет раскодироваться так:
```json
{"List": {"0": "A", "1": "B", "2": "C"}}
```
...
Для списков HELML удобен "особенный ключ следующий номер", он позволяет записать приведённый выше пример так:
```sh
List:
 :--: A
 :--: B
...
* Если в начале ключа есть знак "-", это указывает на особые случаи.
...
- Особый ключ "`-++`" (как и все ключи, начинающиеся на "`-+`"), относятся к управлению многослойностью.
- Все прочие варианты, начинающиеся на "-", декодируются из Base64 (base64url).
...
Обратите внимание:
  ...
  * в этом примере есть ключ `:-:` который преобразовался в ключ "пустая строка".
...
* Особые случаи - когда значение особенное (например, бинарное), или оно имеет не-строковой тип.
...
При кодировании в URL-режиме разделители строк заменяются на "`~`", управляющие двоеточия на "`.`", а управляющие
пробелы на нижнее подчёркивание "_". На выходе получается строка, которая легко может быть использована как GET-параметр для URL,
или как аргумент командной строки. В таком виде HELML-формат визуально менее понятен, чем в многострочном формате,
но декодируется тем же парсером, что и построчный формат.

Выбор символов для замены обусловлен тем, что преобразование "urlencode" (RFC 3986) их не изменяет.
Это простейший способ передать в одной строке произвольный массив с сохранением его структуры и типов данных.
