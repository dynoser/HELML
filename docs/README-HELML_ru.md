...
Если "склеить" все строки HELML при помощи тильды `~`, то парсер будет понимать этот символ как "перевод строки".

Условно назовём это "однострочный HELML". Он отличается от "многострочного HELML" разделителем строк.

Однострочный HELML удобен когда по каким-то причинам нельзя использовать "перевод строки".
Например, если надо поместить HELML-код в URL, или в аргумент командной строки, и т.д.

Это простой способ передать в строковом параметре произвольный массив с сохранением его структуры и типов данных.

Кроме того, есть особый URL-режим, в котором заменяются управляющие двоеточия и пробелы на "`.`" и "`_`" соотвтетсвенно.
Выбор этих символов для замены обусловлен тем, что преобразование "urlencode" (RFC 3986) их не изменяет.
Признаком кодирования в этом режиме является наличие знака `~` в конце строки однострочного HELML.

Таким образом, различаются два варианта однострочного HELML:
 - простой однострочный HELML, когда строки склеены через знак `~`
 - URL-режим HELML, в котором дополнительно заменены двоеточия и пробелы на `.` и `_`.

Все варианты декодируются одним и тем же декодером, используемый вариант кодировки определяется автоматически.
